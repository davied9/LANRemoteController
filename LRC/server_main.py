...
def _register_lrc_commands(command_server, config, commands_kwargs=dict()):
...
def new_entry():
    import sys
    # parse config
    config = parse_config_from_console_line_with_parser(sys.argv[1:])
    # clean arguments
    sys.argv = [sys.argv[0]]
    # start server
    start_lrc_server_new_entry(config)


def start_lrc_server_new_entry(config):
    ...
    if config.enable_ui:
        ...
        if config.commands:
            logger.warning('LRC : UI is enabled, commands {} will not be executed.'.format(config.commands))
        ...
        register_lrc_commands(command_server, config)
        if not command_server.is_running:
            command_server.start()
...
def register_lrc_commands(command_server, config):
    from LRC.Server.Commands.LRCServer import start_lrc, start_lrc_server, start_lrc_waiter
    from LRC.Server.Commands.LRCServer import stop_lrc, stop_lrc_server, stop_lrc_waiter
    from LRC.Server.Commands.LRCServer import quit as quit_lrc
    from LRC.Server.Command import Command
    ...
    lrc_config = dict()
    if config.lrc_server_address:
        lrc_config['server_address'] = eval(config.lrc_server_address)
    if config.lrc_waiter_address:
        lrc_config['waiter_address'] = eval(config.lrc_waiter_address)
    if config.lrc_verify_code:
        lrc_config['verify_code'] = config.lrc_verify_code

    # start/stop LRC
    command_server.register_command('start_lrc', Command(name='start_lrc', execute=start_lrc, kwargs=lrc_config))
    command_server.register_command('stop_lrc', Command(name='stop_lrc', execute=stop_lrc))

    # start/stop LRC server
    command_server.register_command('start_lrc_server', Command(name='start_lrc_server', execute=start_lrc_server, kwargs=lrc_config))
    command_server.register_command('stop_lrc_server', Command(name='stop_lrc_server', execute=stop_lrc_server))

    # start/stop LRC waiter
    command_server.register_command('start_lrc_waiter', Command(name='start_lrc_waiter', execute=start_lrc_waiter, kwargs=lrc_config))
    command_server.register_command('stop_lrc_waiter', Command(name='stop_lrc_waiter', execute=stop_lrc_waiter))
